...
print(f">>>Exempo 01: Dados dos Sensores<<<")
print(f"Soma dos valores ponderados: {soma_valores:.2f}")
print(f"Soma dos pesos: {soma_dos_pesos:.2f}")
print(f"Media ponderada das temperaturas : {media_sensores:.2f}")
    
media_simples = sum(all_data) / len(all_data)
print(f"Media Aritmétrica Simples das Temperaturas: {media_simples:.2f} ºC")
...
array_peso_ativos = [peso_ativos_balanceado] * len(satisfacao_usuarios_ativos)
array_pesos_ocasionais = [peso_ocasionais_balanceado] * len(satisfacao_usuarios_ocasionais)
array_todos_pesos= array_peso_ativos + array_pesos_ocasionais
...
print(f"Média Ponderada de Satisfação do Usuário: {media_satisfacao_ponderada:.2f}")
media_simples = sum(amostra_completa) / len(amostra_completa)
print(f"Média Simples de Satisfação do Usuário: {media_simples:.2f}")